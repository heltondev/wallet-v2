...
export async function handler(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
  try {
    ...
    if (resource === '/recurring/generate' && method === 'POST') return generateRecurring(event, auth.userId);
    if (method === 'POST') return createRecurring(event, auth.userId);
    if (method === 'GET' && !hasId) return listRecurring(event, auth.userId);
    if (method === 'PUT' && hasId) return updateRecurring(event, auth.userId);
    if (method === 'DELETE' && hasId) return removeRecurring(event, auth.userId);
