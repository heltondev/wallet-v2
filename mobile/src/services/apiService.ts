...
import type { AiExtractResult, AiExtractRecurringResult, AiVerifyPaymentsResult, WorkspaceShare } from '../types';
...
export const aiExtractReceipt = async (
  ...
  for (let i = 0; i < 60; i++) {
    ...
    const job = await apiFetch<{ status: string; result?: AiExtractResult; error?: string }>(`/ai/jobs/${jobId}`);
...
export const aiExtractRecurring = async (
  files: { base64: string; mimeType: string }[],
  text: string,
): Promise<AiExtractRecurringResult> => {
  ...
  for (let i = 0; i < 60; i++) {
    await new Promise<void>((r) => setTimeout(r, 5000));
    const job = await apiFetch<{ status: string; result?: AiExtractRecurringResult; error?: string }>(`/ai/jobs/${jobId}`);
    if (job.status === 'completed' && job.result) return job.result;
    if (job.status === 'failed') throw new Error(job.error ?? 'AI processing failed');
  }
...
export const aiVerifyPayments = async (
  files: { base64: string; mimeType: string }[],
  text: string,
): Promise<AiVerifyPaymentsResult> => {
  ...
  for (let i = 0; i < 60; i++) {
    await new Promise<void>((r) => setTimeout(r, 5000));
    const job = await apiFetch<{ status: string; result?: AiVerifyPaymentsResult; error?: string }>(`/ai/jobs/${jobId}`);
    if (job.status === 'completed' && job.result) return job.result;
    if (job.status === 'failed') throw new Error(job.error ?? 'AI processing failed');
  }
...
export const shareWorkspace = (workspaceId: string, data: { email: string; role: string }) =>
  apiFetch<WorkspaceShare>(`/workspaces/${workspaceId}/shares`, { method: 'POST', body: JSON.stringify(data) });

export const listShares = (workspaceId: string) =>
  apiFetch<WorkspaceShare[]>(`/workspaces/${workspaceId}/shares`);

export const updateShare = (workspaceId: string, userId: string, data: { role: string }) =>
  apiFetch<WorkspaceShare>(`/workspaces/${workspaceId}/shares/${userId}`, { method: 'PUT', body: JSON.stringify(data) });
...
export const uploadFileToS3 = async (url: string, fileBody: Blob | ArrayBuffer, contentType: string) => {
  await fetch(url, { method: 'PUT', body: fileBody, headers: { 'Content-Type': contentType } });
};
