...
export function getRecurringStatuses(
  ...
  return recurring
    ...
    .map(r => {
      ...
      if (!paymentMatch) {
        txMatch = thisMonthTx.find(tx => tx.recurringId === r.id);
        if (!txMatch) {
          const descLower = r.desc.toLowerCase();
          txMatch = thisMonthTx.find(tx => {
            if (tx.desc.toLowerCase() !== descLower) return false;
            const ratio = Math.abs(tx.amount) / Math.abs(r.amount);
